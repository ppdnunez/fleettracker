        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
            <div style={{ background: '#111c33', borderRadius: 12, width: 440, maxHeight: '88vh', overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.3)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid #1e2c46' }}>
                    <h2 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: '#eaeff9' }}>{isNew ? 'New Computed Attribute' : 'Edit Computed Attribute'}</h2>
                ...
                <div style={{ padding: 20 }}>
                    ...
                    <div style={{ border: '1px solid #1e2c46', borderRadius: 8, overflow: 'hidden', marginBottom: 14 }}>
                    ...
                    <div style={{ border: '1px solid #1e2c46', borderRadius: 8, overflow: 'hidden' }}>
                        <button onClick={() => setTestOpen(o => !o)} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: '#111c33', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: '#cfdcf0', textTransform: 'uppercase', letterSpacing: 0.4 }}>
                            Test
                        ...
                        {testOpen && (
                            <div style={{ padding: 14, borderTop: '1px solid #1e2c46' }}>
                                ...
                                <select value={testDeviceId} onChange={e => setTestDeviceId(e.target.value)} style={{ ...inputStyle, background: '#111c33', cursor: 'pointer', marginBottom: 10 }}>
                                    {devices.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
                                </select>
                                <button onClick={handleTest} disabled={testing || !testDeviceId} style={{ width: '100%', padding: 9, borderRadius: 7, border: '1.5px solid #3b82f6', background: '#111c33', color: '#3b82f6', fontSize: 13, fontWeight: 600, cursor: testing ? 'not-allowed' : 'pointer' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 760 }}>
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr><td colSpan={5} style={{ ...TD, textAlign: 'center', padding: 48, color: '#5e7094' }}>No data found</td></tr>
                        ) : filtered.map(a => (
                            <tr key={a.id}>
                                <td style={{ ...TD, fontWeight: 500 }}>{a.description}</td>
                                <td style={{ ...TD, fontFamily: 'monospace' }}>{a.attribute}</td>
                                <td style={{ ...TD, fontFamily: 'monospace', color: '#9daec9' }}>{a.expression}</td>
            ...
            {pendingDeleteId && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
                    <div style={{ background: '#111c33', borderRadius: 12, padding: '24px 28px', width: 300, boxShadow: '0 16px 48px rgba(0,0,0,0.25)', textAlign: 'center' }}>
                        <h3 style={{ margin: '0 0 8px', fontSize: 15, fontWeight: 700, color: '#eaeff9' }}>Delete computed attribute?</h3>
