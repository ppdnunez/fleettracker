...
function GroupModal({ group, allGroups, onClose, onSaved }) {
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
            <div style={{ background: '#1a1a1a', borderRadius: 12, width: 420, maxHeight: '88vh', overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.3)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid #2c2c2c' }}>
                    <h2 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: '#f5f0e8' }}>{isNew ? 'New Group' : 'Edit Group'}</h2>
                ...
                <div style={{ padding: 20 }}>
                    ...
                    <div style={{ border: '1px solid #2c2c2c', borderRadius: 8, overflow: 'hidden', marginBottom: 14 }}>
                        ...
                        {extraOpen && (
                            <div style={{ padding: 14, borderTop: '1px solid #2c2c2c' }}>
                                ...
                                <select value={parentId} onChange={e => setParentId(e.target.value)} style={{ ...inputStyle, background: '#1a1a1a', cursor: 'pointer' }}>
                    ...
                    <div style={{ border: '1px solid #2c2c2c', borderRadius: 8, overflow: 'hidden' }}>
                        <button onClick={() => setAttributesOpen(o => !o)} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: '#1a1a1a', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: '#d5c9b8', textTransform: 'uppercase', letterSpacing: 0.4 }}>
                            Attributes
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#1a1a1a', position: 'relative' }}>
            ...
            <div style={{ flex: 1, overflow: 'auto', padding: '12px 20px 16px' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    <tbody>
                        {loading ? (
                            <tr><td colSpan={2} style={{ ...TD, textAlign: 'center', padding: 48, color: '#5a4e42' }}>Loading…</td></tr>
                        ) : filtered.length === 0 ? (
                            <tr><td colSpan={2} style={{ ...TD, textAlign: 'center', padding: 48, color: '#5a4e42' }}>No data found</td></tr>
                        ) : filtered.map(g => (
                            <tr key={g.id}>
            ...
            {pendingDeleteId && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
                    <div style={{ background: '#1a1a1a', borderRadius: 12, padding: '24px 28px', width: 300, boxShadow: '0 16px 48px rgba(0,0,0,0.25)', textAlign: 'center' }}>
                        <h3 style={{ margin: '0 0 8px', fontSize: 15, fontWeight: 700, color: '#f5f0e8' }}>Delete group?</h3>
